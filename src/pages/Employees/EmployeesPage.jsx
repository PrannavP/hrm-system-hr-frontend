...
import { deleteEmployee, employeesList } from "../../services/api";
...
const EmployeePage = () => {
	...
	const [departmentOptions, setDepartmentOptions] = useState(["All"]); // Initialize with "All"
	...
	useEffect(() => {
		const fetchEmployees = async () => {
			try {
				...
				// Extract unique departments and update departmentOptions
	...
	const deleteEmployeeFnc = async (emp_id) => {
		try {
				...
				{popupVisible && (
					...
					>
						<ul className="py-1">
							<li>
								<Link
									to={`/employee/${selectedEmployee?.emp_id}`}
